...
We selected the following set of variables to represent the effect of climate: mean annual temperature (BIO1), mean annual precipitation (BIO12), annual precipitation variability (BIO15) over the period 1981-2010. These bioclimatic variables were extracted from CHELSA V2.1 [@Krager2017]. These variables were all highly correlated in our dataset (Figure S\@ref(fig:choice-clim-vars)a). We performed a principal component analysis after scaling and centering these three variables: the first component explained `r round(pca_clim$sdev[1]^2/sum(pca_clim$sdev^2)*100)`% of the variance in our dataset (Figure S\@ref(fig:choice-clim-vars)b). We therefore decided to keep only one variable to represent this climatic gradient: mean annual temperature, which was most correlated with the first component of the PCA (Figure S\@ref(fig:choice-clim-vars)b), and has been shown to influence C stocks in other contexts [@Wiesmeier2019;@Muller-Landau2021]. 

We selected the following set of variables to represent the effect of soil properties on C stocks: clay fraction (<2 $\mu$m), silt fraction (2-50 $\mu$m), sand fraction (50-2000 $\mu$m), pH and cation exchange capacity xx. These edaphic variables were measured on our soil samples analysed in the Cirad laboratories (see section "Field inventories"). After scaling and centering these variables, we performed a principal component analysis: the first component explained `r round(pca_soil$sdev[1]^2/sum(pca_soil$sdev^2)*100)`% of the variance in our data set (Figure S\@ref(fig:choice-soil-vars)b). We therefore decided to retain only one variable to represent this edaphic gradient: clay fraction, which was one of the most highly correlated with the first component of the PCA (Figure S\@ref(fig:choice-soil-vars)b) and has been shown to influence C stocks in other contexts [@Wiesmeier2019]. 
...
```{r fig.id = "dag", fig.cap="Direct acyclic plot of the hypothesised effect of soil characteristics, climate and land use (here AFS) on AGC and SOC stocks.", fig.lp="supp-fig", results = "show", fig.cap.pre="Figure S"}
# save_png <- function(plot, path){
#   DiagrammeRsvg::export_svg(plot) %>%
#     charToRaw() %>%
#     rsvg::rsvg() %>%
...
```{r fig.id="choice-clim-vars", fig.width = 10, fig.height=4, fig.cap="(a) Pearson coefficient of correlation matrix and (b) principal component analysis biplot, with three climate variables: mean annual temperature (MAT), mean annual precipitation (MAP) and precipitation variability (PVar). Values for the climate variables were extracted from CHELSA V2.1 (Karger et al., 2017) for each of the 30 study plots.", fig.lp="supp-fig", results = "show", fig.cap.pre="Figure S"}
...
```{r fig.id = "choice-soil-vars", fig.width = 10, fig.height=4, fig.cap="(a) Pearson coefficient of correlation matrix and (b) principal component analysis biplot, with five soil variables: clay, silt and sand fraction, soil pH, and cation exchange capacity (CEC). Values for the soil variables were measured from in situ soil samples for each of the 30 study plots.", fig.lp="supp-fig", results = "show", fig.cap.pre="Figure S"}
