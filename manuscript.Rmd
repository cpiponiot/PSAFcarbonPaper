...
# test normality
dataC[, .(
  shap.A = shapiro.test(log(meanAGC))$p.value, 
  shap.S = shapiro.test(log(meanSC))$p.value
  ), .(ID_AF, ID_SUCO)]
# levene test of homoscedasticity
library(car)
dataC[, leveneTest(log(meanAGC) ~ ID_AF*ID_SUCO)]$`Pr(>F)`[1]
dataC[, leveneTest(log(meanSC) ~ ID_AF*ID_SUCO)]$`Pr(>F)`[1]
...
# 2-way anova 
dataC[, ID_SUCO := substr(ID_VILLAGE, 1, 1)]
anov1 = anova(lm(log(meanAGC)~ID_SUCO+ID_AF, data = dataC))
anov2 = anova(lm(log(meanSC)~ID_SUCO+ID_AF, data = dataC))
...
# anov2 = anova(lm(rank(meanSC)~ID_SUCO+ID_AF, data = dataC))
# tab_anov = data.frame(response = rep(c("AGC", "SOC"), each = 2), 
#                       ind_var = rep(c("SUCO", "AF"), 2), 
#                       p = round(c(anov1$`Pr(>F)`[1:2], anov2$`Pr(>F)`[1:2]), 3)
# )
...
lmeReg <- lmerTest::lmer(log(meanSC) ~ log(meanAGC) + (1|village), data = dataC)
...
setDT(dfpred)
dfpred[, meanSC := exp(intercept + slope * log(meanAGC))]
...
ggplot(dataC_fig, aes(color = ID_AF)) +
  ...
  geom_line(data = dfpred, aes(x = meanAGC, y = meanSC), lty = 2, col = 1) +
...
Total carbon stocks varied between `r dataC[, round(min(meanAGC + meanSC + meanBGC, na.rm = TRUE))]` and `r dataC[, round(max(meanAGC + meanSC + meanBGC, na.rm = TRUE))]` Mg C/ha, with an average of `r dataC[, round(mean(meanAGC + meanSC + meanBGC, na.rm = TRUE))]` Mg C/ha and a coefficient of variation of `r dataC[, round(100*sd(meanAGC + meanSC + meanBGC, na.rm = TRUE)/mean(meanAGC + meanSC + meanBGC, na.rm = TRUE))]`%  (Table 1; Figure 2). The AFS with the lowest total carbon stocks was the `r dataC[, mean(meanAGC + meanSC + meanBGC, na.rm = TRUE), .(ID_AF)][, ID_AF[which.min(V1)]]` with an average of `r dataC[, mean(meanAGC + meanSC + meanBGC, na.rm = TRUE), .(ID_AF)][, round(min(V1))]` Mg C/ha; the AFS with the highest total carbon stocks was the `r dataC[, mean(meanAGC + meanSC + meanBGC, na.rm = TRUE), .(ID_AF)][, ID_AF[which.max(V1)]]` with an average of `r dataC[, mean(meanAGC + meanSC + meanBGC, na.rm = TRUE), .(ID_AF)][, round(max(V1))]` Mg C/ha (Table 1; Figure 2).
...
SOC stocks varied between `r dataC[, round(min(meanSC, na.rm = TRUE))]` and `r dataC[, round(max(meanSC, na.rm = TRUE))]` MgC/ha, with an average of `r dataC[, round(mean(meanSC, na.rm = TRUE))]` MgC/ha and a coefficient of variation of `r dataC[, round(100*sd(meanSC, na.rm = TRUE)/mean(meanSC, na.rm = TRUE))]`%  (Table 1; Figure 2). In disagreement with our hypothesis (H1b), no significant differences in SOC were observed between the five AFS studied (ANOVA p-value: `r tab_anov$p[tab_anov$response=="SOC" & tab_anov$ind_var == "AF"]`). However, the measured SOC stocks differed from suco to suco (ANOVA p-value: `r tab_anov$p[tab_anov$response=="SOC" & tab_anov$ind_var == "SUCO"]`; Table 2). The SOC stocks in Gariuai were on average higher (`r dataC[ID_SUCO=="G", round(mean(meanSC, na.rm = TRUE))]` Mg C/ha) than those in Osso Luga (`r dataC[ID_SUCO=="S", round(mean(meanSC, na.rm = TRUE))]` Mg C/ha; Figure 2).
...
```{r sfig-altitude}
dataCelev = merge(dataC[, c("N_PLOT", "meanSC", "uprSC", "lwrSC")], 
                 data_plot[, c("N_PLOT", "elev", "village")], by = "N_PLOT")

ggplot(dataCelev, aes(x = elev, y = meanSC, col = village)) +
  geom_pointrange(aes(ymin=lwrSC, ymax = uprSC)) + 
  theme_classic() +
...
# simulation 100 values per plot, soil depth and variable with a 10% coefficient
# of variation (CV value from lab)
data_soil_plot_simu = 
  data_soil_plot[, .(
    value = rnorm(100, value, sd = 0.1*value), n = 1:100), 
