...
**Word count**: 7200 (with SI); **Figure and table count**: 5 + 6 in SI
...
# group total C stocks in plots 6 and 8 (otherwise will underestimate CF total C
# stocks); this will not be used for analysis, only to display average C stocks
# per AFS
# data_sim <- rbind(
#   data_sim[variable != "TOT" | !N_PLOT %in% c(6, 8)], 
#   data_sim[variable == "TOT" & N_PLOT %in% c(6, 8), .(value = sum(value), N_PLOT = 6), .(site, ID_AF, variable, iter)]
# )
```
...
## Large C stocks in traditional AFS in Timor-Leste

Compared to values reported in the literature for other AFS, the carbon stocks in this study were particularly high. The value of SOC stocks was similar to other reported values [@Feliciano2018], but some of the AFS studied here have particularly high stocks in their biomass [compared to the average of 74\ Mg\ C\ ha$^{-1}$ in @Ma2020]. The AFS with the highest carbon stocks, forest gardens, had stocks close to those of old-growth tropical forests [270\ Mg\ C\ ha$^{-1}$; @Lal2005]. These high carbon stocks could be explained by the fact that forest gardens have generally grown for more than 50 years, with little human intervention (Table S1). In terms of structure and carbon stocks, forest gardens should therefore resemble old secondary forests, which have been shown to recover more than 60% of the AGC stocks and almost all the SOC stocks of an old forest [@Poorter2021]. In addition, FGs are generally located close to villages, where soils are enriched by organic waste and animal manure, which can enhance tree growth [@Madejon2016].
...
The AFS studied here vary considerably in terms of AGC stocks, in agreement with our hypothesis H1a. This reflects the density and size of trees in the plot, which in turn depends on the importance of trees in the production system. In some systems, such as CF and SP, trees do not contribute to food production but have other functions, such as restoring soil fertility (e.g. during the fallow period) or providing shade for livestock. This more limited role for trees may explain why their total biomass is lower in these systems: in the CF system, the plot is cleared and annual crops are planted in full sun, while in the SP system, the growth of grazing grasses is favored by reducing the number of trees that shade them.

It is also interesting to note that, in line with previous results, the age of the trees in the AFS largely explains the carbon stored in their biomass [@Ma2020]. Indeed, the systems described in this study reflect different stages of transition in land use, which follow the growth of their trees. For example, a farmer may plant trees in a CF field, which will then become a YA with trees aged between one and ten years, where the main production still comes from annual crops. When the trees are mature and producing fruit, the YA becomes a HG or a FG. In the latter systems, the trees become the main source of production, both for food and wood.

The location of AFS in the landscape is not random and could also explain the observed differences in AGC stocks. For example, most HGs are located close to houses, in alluvial valleys that are well supplied with water and where the soil is enriched with organic waste and animal manure, which is likely to increase soil fertility [@Palm2001]. The same applies to FGs, which are former HGs that have been more or less abandoned, and YAs, where the farmer has chosen the site with the intention of turning it into a HG. We therefore expect HGs, FGs and YAs to be located in areas that are more favorable for tree growth [@Wagner2012;@Madejon2016;@Treuer2018]. Conversely, CF and SP fields are mainly located on limestone plateaus where water is deep and soils are shallow, which could reduce tree growth and biomass [@Imada2008].

## Importance of historical and environmental factors on SOC 

Our results revealed that SOC was mainly sensitive to the differences in location and villages rather than AFS systems in each location. This was also reported by the meta-analysis of @Feliciano2018 who highlighted that only 3.2% of the variance of the soil carbon sequestration was explained by agroforestry system choices. The two villages studied were different in terms of inherent soil properties (Table S\@ref(tab:site-soil); Figure S\@ref(fig:clay-corg)), altitudes (Figure \@ref(fig:map-sites); Figure S\@ref(fig:altitude-corg)) and also history. All those driving factors independent of the AFS management could not be disentangled in this study but their cumulative effects were clearly observed. The difference in terms of soil texture and especially clay content was reported to be a key driving process in terms of SOC storage in the tropics [@Don2010;@Matus2021]. In this study, the fine fractions of soil texture were however not linked to the SOC stocks (Figure S\@ref(fig:clay-corg)). In addition to soil inherent conditions, other key driving factors for SOC changes, independent to the AFS management practices, may have led to those differences between. Gariuai was inhabited whereas Osso-Luga was not for a long time. The management and care of the plot in Gariuai such as pruning, livestock visits, optimizing the different stratas in terms of carbon input could have led to increased soil organic carbon compared to abandoned systems [@Lorenz2014]. 

The AGC stocks differed from one system to another but we could not detect any significant difference in SOC between AFSs. This observation was in contradiction with our H1b. Our systems were rather contrasted and previous studies reported significant differences in SOC sequestration between different AFS [@Ma2020;@Kimaro2024]. Local conditions and the rapid SOC mineralisation in tropical areas may explain this lack of sensitivity against AFs management practices [@Sayer2019]. Our experimental design with a limited number of replicates within each village could have also limited the significance of the tests. Some trends were highlighted, such as higher SOC in SP in Gariuai. The design was based on 3 true replicates for each of the systems in each village with the objective to limit as much as possible the possible influence of external co-factor in each village (e.g. farmer management practices variability, micro-topography, etc). However, much variability cannot be controlled in farmers’ plots and increasing the number of replicates for each AFS may have strengthened the power of the tests and may have led to more differences between systems.  

## Lack of relationship between soil and biomass carbon

We found no relationship between AGC and SOC stocks both in the topsoil layer 0-10 cm depth (H2a refuted) and in 0-30 cm depth (H2b confirmed).

These results could be explained by previous findings that SOC stocks saturate much faster than AGC stocks in tropical AFS. For example, in a global meta-analysis, @Ma2020 estimated that it took only 5 years for SOC stocks in tropical AFS to reach an equilibrium value, but more than three decades for AGC stocks. This desynchronisation of SOC and AGC stocks recovery has also been observed in naturally regenerating tropical forests. In a systematic review, @Martin2013 found that SOC in tropical secondary forests changed little with time since disturbance, while AGB increased. @Ojoatre2024 found similar results in a tropical montane secondary forest. These results imply that there is little relationship between AGC and SOC once agroforestry systems are more than 5 years old, which is the case in our study for FGs, HGs (> 5 years old) and partially for YAs (2 to 10 years; Table S\@ref(tab:afs-typology)). This saturation of SOC even with increasing AGC is counterintuitive, as it might be expected that greater AGC would lead to greater litter production and decomposition, some of which would increase SOC. However, this was not shown by @Sayer2019, who found that SOC did not increase in a lowland tropical forest in Panama after 15 years of an experimentally doubled aboveground litter input. Two processes could explain this. On the one hand, the addition of easily decomposable litter could stimulate microbial decomposition and turnover of old stored SOC [i.e. priming effect; @Sayer2019]. On the other hand, the capacity of the soil to store SOC through organo-mineral associations would be limited by its fine element content (clay + fine silt) (i.e. saturation concept).

## How to conserve and improve carbon stocks in traditional AFS in Timor-Leste?  

Although agroforestry can make agriculture more resilient while improving its carbon footprint and biodiversity conservation value [@Cardinael2021], poor adaptation to the local context can explain many failures to adopt new agroforestry techniques [@Coe2014]. Conversely, some traditional agroforestry techniques have evolved in a particular context (biophysical, social and economic) and are therefore adapted to it [@Gouyon1993;@Aumeeruddy1994;@Peltier1996]. This is the case of the traditional AFS studied here: their heritage and socio-economic values [as shown by @Cogne2024], in addition to their high carbon stock value, confirm the importance of their conservation, both nationally and internationally.

Conservation of these traditional AFS should avoid their abandonment as the trees wither or as their production value declines. Clear-cutting old agroforests for full-sun crops may then become an attractive short-term strategy, providing a large cash inflow from the sale of wood products, followed by 4 or 5 years of agricultural production on fertile land. Unfortunately, this results in a significant loss of biodiversity and carbon stored in the biomass, and can lead to an irreversible loss of fertility on the plot through soil degradation and erosion. This phenomenon has been documented in a similar context in Sumatra (Indonesia), where @Gouyon1993 highlighted the fact that old agroforests, which they called "jungle-rubber" and which are quite similar in structure to "forest gardens" (except for the rubber tree component, which has been replaced by other species in Timor), were being cleared in favor of more intensive production systems. This trend towards clearing jungle rubber was subsequently confirmed by @Ekadinata2011, and its impact on carbon stock decline was quantified by @Villamor2014.

Strategies to avoid clearing of these traditional AFS could be to develop new ways of intensifying production while maintaining high carbon and biodiversity values, and gradually renewing the tree population in the plots. For example, it would be interesting to develop renewal through clearings of about one hundred square meters (corresponding to the felling of 1 to 10 trees, depending on their crown area). In these clearings, young trees of species considered interesting for their future production would be associated, first with light crops (cereals, etc.) and then with crops of shady species (taro, etc.). Additional mechanisms to encourage the renewal of traditional AFS could be the provision of subsidies (e.g. payment of carbon credits) or low-interest loans to farmers wishing to renew their agroforestry plots. In addition, a "sustainable agriculture" type label could be awarded to these AFS products, and their value chains could be supported by governments and international certification bodies, as is already the case for coffee in many countries [@Bertrand2019].

# Conclusion

Agroforestry systems in the Baucau district of Timor-Leste had variable but overall high carbon stocks. These systems have only recently been described in the scientific literature, and much remains to be done to improve our knowledge of their diversity and functioning. It is therefore important that all stakeholders including local authorities, universities and NGOs, take into account their existence, diversity and environmental and social value, in order to propose possible innovations to maintain or improve their functioning.

# Authors' contribution 

All authors contributed to the design of the sampling protocol. MC carried out the fieldwork with the help of RP, AT and VF [mention students?]. CP analyzed the data with the help of VF and MC. CP prepared the manuscript and all authors contributed to the drafting and revision of the manuscript. 
...
```{r tab.id = "site-soil", tab.lp="supp-tab", results = "show", tab.cap.pre="Table S", tab.cap = "Soil characteristics of the sites (average over depths 0-30 cm), presented as mean values with 95% confidence intervals (in parentheses). The confidence intervals were calculated by propagating a 12% error on the lab-provided values and obtaining site-level estimates through bootstrapping plot values (1000 simulations)."}
flextable(
  data_soil_site,
...
```{r tab.id = "afs-typology", tab.cap.pre="Table S", tab.cap = "Description of 5 agroforestry systems in Central Timor-Leste [adaptation of the name given by Nair, position in the landscape, surface area, age since creation, previous cultivation", tab.autonum.start_at=1, tab.lp="supp-tab", results = "show"}
...
flextable(vern) |> 
  set_header_labels(values =  c("Tetum name", "Scientific name")) |> 
  autofit()
```

## Figures 

```{r fig.id="design", fig.cap = "Plot sampling design. (a) Typical plot < 1 ha, in all AFS except SP. (b) Typical 1-ha plot in SP systems. Red areas are the five 10x10 m subplots, and blue dots are the three soil sampling locations.", fig.autonum.start_at=1, fig.lp="supp-fig", results = "show", fig.cap.pre="Figure S", fig.width = 5, fig.height = 3}
include_graphics("figures/plot-design.jpg") 
```

```{r fig.id = "clay-corg", fig.height = 4, fig.width = 6, fig.lp="supp-fig", fig.cap="Distribution of SOC content (y-axis; in %) values as a function of fine silt and clay content (x-axis; in %), soil depth (point color; in cm), and site (point shape).", fig.cap.pre="Figure S"}
...
```{r fig.id="altitude-corg", fig.height = 4, fig.width = 6, fig.lp="supp-fig", fig.cap = "Effect of plot elevation (x-axis; in m) on SOC stocks (y-axis; in\ Mg\ C\ ha^-1^). Points are colored by site; the point corresponds to the mean prediction at that site, while the error bars correspond to the 95% confidence interval.", fig.cap.pre="Figure S"}
...
lmer(log(mean) ~ log(elev) + (1|site), data = dataCelev) |> 
  summary()
