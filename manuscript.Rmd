...
author: "Camille Piponiot*, Marguerite Cogné*, Vincent Freycon, Alexis Toumazeau, Régis Peltier"
date: "2024-03-19"
output: 
...
The research was carried out in the center of the Baucau district, on the north-eastern coast of Timor-Leste. Two communities, or suco (group of villages), were selected to represent two contrasting topo-geographical, climatic and historical cases: (i) Gariuai suco (long-lat coordinates: `r site_summary[site=="Gariuai", coord]`) is located on a plateau of Baucau limestone, with an annual rainfall of `r site_summary[site=="Gariuai", prec]` mm and an altitude ranging 300-750 m; and (ii) the village of Osso Luga in Samalari suco (long-lat coordinates: `r site_summary[site=="Osso Luga", coord]`) is located on the Matebian foothill based on Bobonaro Scaly Clay, with an annual rainfall of `r site_summary[site=="Osso Luga", prec]` mm and an altitude ranging `r site_summary[site=="Osso Luga", elev]` m (Figure\ \@ref(fig:map-sites)). The soils of the Baucau district are classified using the WRB soil classification [@Mantel2023] as Lithosols and Calcic Luvisols [@FAO2012]. The average soil properties differed between villages (Table\ S\@ref(tab:site-soil)). Gariuai had a clay loam texture whereas Osso Luga had a clay texture. The two villages also have had different histories. Gariuai has been inhabited for at least 50 years, while Osso Luga was abandoned during the Indonesian occupation, and was only repopulated after 2003. During the period of abandonment, the various agroforestry systems in Osso Luga lay fallow.

## Typology of agroforestry systems

We used the typology described in @Cogne2024, developed in the same study area. This typology was based on the one proposed by Nair et al. (1993, 2021) and adapted to the local context with semi-structured interviews conducted in the same study area [@Cogne2024]. The five AFSs identified were: Crop system including a wooded Fallow phase (CF), Silvopasture (SP), Young Agroforest (YA), Home Garden (HG) and Forest Garden (FG). CFs are plantations of a few non-perennial crops (such as maize, sweet potatoes, groundnuts and squash) which are left fallow after 1 to 3 years. SPs consist in a vast area (generally 200 to 500 ha) where cattle are grazed under a layer of trees. YAs are agricultural plots where the farmer has been planting rows of trees, spaced at around 5x10m, for two to 10 years, usually with the help of projects. HGs are plots of land close to houses, where crops and livestock (poultry, pigs, cattle, etc.) have been grown in the shade of numerous trees and palms for multiple uses (wood, fruit, fibre, etc.) for more than five years. FGs are generally former HGs (over 50 years old), which have been abandoned, for example as a result of the war, and are now used mainly for gathering with limited human influence.

The AFSs are presented in more detail in the supplemental information (Table\ S\@ref(tab:afs-typology)) and in @Cogne2024. 

## Field inventories

We randomly selected 3 plots from each of the 5 traditional AFSs identified in each of the two sites (30 plots in total; Figure\ \@ref(fig:map-sites)). Sampling plots were defined as the entire agricultural plot (area between 0.06-1.25 ha) for all AFSs except SP plots, which were generally much larger than 1 ha. In SP plots, a 1 ha sampling plot was delineated within the agricultural plot. Tree and soil measurements (described below) were carried out between June and August 2021. 
...
In each of the 30 sample plots, three composite samples were taken from three of the five 10 x 10 m subplots, following a transect covering the within plot variability and at a minimum distance of 1 m from any tree (Figure\ S\@ref(fig:design)). Soils were sampled with an auger at 0-10 cm and 20-30 cm. Soils samples were then air-dried and sieved at 2 mm. They were subjected to standard physical and chemical analyses at the Cirad laboratories in Montpellier, France (Table\ S\@ref(tab:site-soil)). Among these analyses, total organic carbon was determined by dry combustion and measured with a thermal conductive detector.

In addition, at one location at the center of the plot, soils were sampled with a cylinder of 250 cm$^3$ at 2-7 cm, 12-17 cm and 22-27 cm. Each sample was dried at 105°C in a classic oven until reaching a stable dry mass, sieved to 2 mm, and then the fine fraction (< 2 mm) was weighed. 
...
All statistical analyses were carried out using R [@R2023].

### Statistical tests

The estimated AGC and SOC stocks were log-transformed for all statistical tests, because we expect them to be log-normally distributed as they only take positive values. We removed one CF plot at Gariuai from the following statistical tests, as it did not contain any trees exceeding the minimum measurement size (CBH = 30 cm) and its AGC stocks were therefore estimated at 0\ Mg\ C\ ha$^{-1}$ (even if there were trees < 30 cm CBH) and could not be log-transformed. After testing for normality (Shapiro-Wilks test) and homoscedasticity (Levene’s test), we performed a two-way ANOVA on the log-transformed estimates of AGC and SOC stocks, grouped by AFS and site. To test the relationship between log-transformed SOC and AGC stocks, we fitted two linear mixed models. We predicted SOC stocks at two different depths (either 0-10 or 0-30 cm) as a function of AGC stocks (fixed effect) and site (Samalari or Osso-Luga; random effect).

### Uncertainty propagation

The uncertainty on plot-level AGC was calculated with the AGBmonteCarlo() function from the BIOMASS package, using a (rather conservative) coefficient of variation of 15% on height measurements [@Hunter2013]; large and small errors on 5 and 95% of trees respectively [@Chave2004;@Rejou-Mechain2017].
To propagate uncertainty on SOC estimates, we used a coefficient of variation of 12% for organic carbon (Cirad laboratory in Montpellier, France), 10% for the mass of the fine fraction [@Page1999]. 
...
Total carbon stocks varied between `r dataC_plot[variable=="TOT", round(min(mean))]` and `r dataC_plot[variable=="TOT", round(max(mean))]`\ Mg\ C\ ha$^{-1}$, with an average of `r dataC_plot[variable=="TOT", round(mean(mean))]`\ Mg\ C\ ha$^{-1}$ and a coefficient of variation of `r dataC_plot[variable=="TOT", round(sd(mean)/mean(mean)*100)]`%  (Table\ \@ref(tab:summary); Figure\ \@ref(fig:barplot)). The AFS with the lowest total carbon stocks was the `r dataC_afs[variable=="TOT", afs[which.min(mean)]]` with an average of `r dataC_afs[variable=="TOT", round(mean[which.min(mean)])]`\ Mg\ C\ ha$^{-1}$; the AFS with the highest total carbon stocks was the `r dataC_afs[variable=="TOT", afs[which.max(mean)]]` with an average of `r dataC_afs[variable=="TOT", round(mean[which.max(mean)])]`\ Mg\ C\ ha$^{-1}$ (Table\ \@ref(tab:summary); Figure\ \@ref(fig:barplot)).

## Aboveground carbon

AGC stocks varied between `r dataC_plot[variable=="AGC", round(min(mean))]` and `r dataC_plot[variable=="AGC", round(max(mean))]`\ Mg\ C\ ha$^{-1}$, with an average of `r dataC_plot[variable=="AGC", round(mean(mean))]`\ Mg\ C\ ha$^{-1}$ and a coefficient of variation of `r dataC_plot[variable=="AGC", round(sd(mean)/mean(mean)*100)]`%  (Table\ \@ref(tab:summary); Figure\ \@ref(fig:barplot)). AGC stocks varied significantly between AFS (ANOVA p-value: `r subset(anova_table, variable=="AGC" & factor == "afs")$pvalue`, Table\ \@ref(tab:anova)), but not between site (ANOVA p-value = `r subset(anova_table, variable=="AGC" & factor == "site")$pvalue`, Table\ \@ref(tab:anova)). The AFS with the lowest AGC stocks was the `r dataC_afs[variable=="AGC", afs[which.min(mean)]]` with an average of `r dataC_afs[variable=="AGC", round(mean[which.min(mean)])]`\ Mg\ C\ ha$^{-1}$; the AFS with the highest AGC stocks was the `r dataC_afs[variable=="AGC", afs[which.max(mean)]]` with an average of `r dataC_afs[variable=="AGC", round(mean[which.max(mean)])]`\ Mg\ C\ ha$^{-1}$ (Table\ \@ref(tab:summary); Figure\ \@ref(fig:barplot)).
...
The relationship between AGC and SOC was not significant (p-value = `r round(dfreg$pvalue[grepl("30", dfreg$depth)], 2)`; Figure\ \@ref(fig:plot-lmereg)). This relationship remained insignificant (p-value = `r round(dfreg$pvalue[grepl("10", dfreg$depth)], 2)`) when tested with SOC stocks in the topsoil layer (0-10 cm). The proportion of variance explained by the linear mixed models were `r round(dfreg$R2_cond[grepl("30", dfreg$depth)]*100)`% and `r round(dfreg$R2_cond[grepl("10", dfreg$depth)]*100)`% for 0-30 and 0-10 cm depths respectively, but a marginal R$^2$ of just `r round(dfreg$R2_marg[grepl("30", dfreg$depth)]*100, 1)`% and `r round(dfreg$R2_marg[grepl("10", dfreg$depth)]*100, 1)`% respectively when only the fixed effect was considered (Figure\ \@ref(fig:plot-lmereg)). Overall, AGC stocks were a poor predictor of SOC stocks.

# Discussion 

## Traditional AFS in Timor Leste can store large C stocks

In comparison to values reported in the literature for other AFS, the carbon stocks in this study were relatively high. Specifically, average SOC stocks (`r dataC_plot[variable=="SOC", round(mean(mean))]`\ Mg\ C\ ha$^{-1}$) were higher than other reported values: in a meta-analysis by @Shi2018, it was estimated that AFS stored around 50\ Mg\ C\ ha$^{-1}$ at 0-30 cm depth. These large SOC stocks could be due to the high soil pH associated with high exchangeable Ca content at our sites (Table\ \@ref(tab:site-soil)), as Ca is frequently attributed to enhanced organic matter stabilization [@Wiesmeier2019]. Average biomass stocks (`r dataC_plot[variable%in%c("AGC", "BGC"), sum(mean), .(plot)][, round(mean(V1))]`\ Mg\ C\ ha$^{-1}$), in contrast, were more similar to the average of 74\ Mg\ C\ ha$^{-1}$ estimated by @Ma2020, but some AFS had particularly high biomass values. The AFS with the highest carbon stocks, `r dataC_afs[variable=="TOT", afs[which.max(mean)]]`s, had total carbon stocks (`r dataC_afs[variable=="TOT", round(max(mean))]`\ Mg\ C\ ha$^{-1}$) similar to those of old-growth tropical forests [270\ Mg\ C\ ha$^{-1}$, @Lal2005]. The high carbon stocks in FGs can be attributed to their long period since establishment, which is generally more than 50 years, with little human intervention (Table\ S\@ref(tab:afs-typology)). In terms of structure and carbon stocks, FGs should therefore resemble old secondary forests, which have been shown to recover more than 60% of the AGC stocks and almost all the SOC stocks of an old forest [@Poorter2021]. In addition, FGs contain a large number of trees of the same age that were planted when the AFS was established and can survive to reach significant sizes, similar to forest plantations that can grow to old-growth-forest levels of AGC stocks after only a few decades [@Brown2020;@Njoukam1996].

Although agroforestry has been identified as one of the possible levers to increase carbon storage in the Timorese agricultural landscape [@UNFCCC2022;@Paudel2022], to our knowledge no study has quantified carbon stocks in these systems. Our study could therefore help to better quantify the national contribution of agroforestry to Timor-Leste's climate change mitigation commitments [@UNFCCC2022], and the potential carbon gains from wider adoption of these techniques. Further studies should help to increase the representativeness of the AFS sampled by including a greater diversity of AFS and environmental conditions. Another critical aspect is the quantification of temporal changes in carbon stocks, which could be assessed in future studies either by having chronosequences of plots where the time since installation is known, or by remeasuring plots through time. 

## AFS differ in C stored in their biomass 

The AFS studied here vary significantly in terms of AGC stocks, in agreement with our hypothesis H1a. This reflects the density and size of trees in the plot, which in turn depends on the importance of trees in the production system. In some systems, such as CF and SP, trees do not contribute to food production but have other functions, such as restoring soil fertility (e.g. during the fallow period) or providing shade for livestock. This more limited role for trees may explain why their total biomass is lower in these systems: in the CF system, the plot is regularly cleared and annual crops are planted in full sun, while in the SP system, the growth of grazing grasses is favored by reducing the number of trees that shade them.

It is also interesting to note that, in line with previous results, the age of the trees in the AFS largely explains the carbon stored in their biomass [@Ma2020]. Indeed, the systems described in this study reflect different stages of transition in land use, which follow the growth of their trees (Figure\ S\@ref(fig:seq-diagram)). For example, a farmer might plant trees in a CF field, which will then become a YA that can still produce annual crops for the first 2 to 4 years, before the shade from the trees becomes too great. The trees are then used as property markers and, when they reach maturity, to produce wood and fruit: the YA then becomes a HG or a FG. Likewise, our results show that AGC stocks increase from CF to YA, HG and FG (Table\ \@ref(tab:summary)).

The location of AFS in the landscape is not random and could also explain the observed differences in AGC stocks. For example, most HGs are located close to houses, in alluvial valleys that are well supplied with water and where the soil is enriched with organic waste and animal manure, which is likely to increase soil fertility [@Palm2001]. The same applies to FGs, which are former HGs that have been more or less abandoned, and YAs, where the farmer has chosen the site with the intention of turning it into a HG. We therefore expect HGs, FGs and YAs to be located in areas that are more favorable for tree growth [@Wagner2012;@Madejon2016;@Treuer2018]. Conversely, CF and SP are mainly located on limestone plateaus where water is deep and soils are shallow, which could reduce tree growth and biomass [@Imada2008].

## Historical and environmental factors explain differences in SOC stocks

We found no significant difference in SOC between AFS (H1b rejected). This result is inconsistent with our expectation that agroforestry practices would influence SOC stocks [@Minasny2017;@Wiesmeier2019] and with previous studies showing significant differences in SOC stocks between different AFSs [e.g., @Ramesh2015]. However, it is worth noting that some single-site studies [e.g. @Betemariyam2020;@GamaRodrigues2010] and at least two meta-analyses failed to find a strong effect of AFS on SOC stocks: @Feliciano2018 highlighted that only 3.2% of the variance in soil carbon sequestration was explained by AFS selection, and @Shi2018 found no significant differences in SOC stocks between 4 different AFSs in 354 previous studies. We offer three main explanations to reconcile these differences between studies. First, in a global meta-analysis, @Ma2020 estimated that it takes only 5 years for SOC stocks in tropical AFS to reach an equilibrium value. In our study, at least three of the five AFS studied (namely YAs, HGs and FGs) represent different stages of an AFS transition depending on the age of the trees (Figure\ S\@ref(fig:seq-diagram). Most of these AFSs are older than 5 years and may have already reached their equilibrium SOC stocks, which could explain why there is little difference between them. Second, we had a limited number of replicates of each AFS within each site, which certainly reduced the power of statistical tests. We were only able to identify trends, such as higher SOC in SPs in Gariuai, which were similar to previous studies [@Feliciano2018]. Increasing the number of replicates for each AFS could have increased the power of the tests and potentially led to more significant differences between AFSs. Third, other pedoclimatic and historical factors influence carbon stocks, sometimes to a greater extent than AFS, e.g. within-site variability in climate, soil texture (Figures\ S\@ref(fig:clay-corg), S\@ref(fig:altitude-corg)) or previous land use may have significantly influenced SOC stocks [@Wiesmeier2019;@Ma2020]. Our experimental design aimed to limit as much as possible the influence of external co-factors at each site (e.g. variability in farmers' management practices, micro-topography, etc.), but much of the variability cannot be controlled in farmers' plots.

We found a significant difference in SOC between the two sites: SOC stocks were higher in Gariuai than in Osso Luga. A priori, three factors that differentiate the sites could explain these differences in SOC stocks: climate (through elevation; Figure\ \@ref(fig:map-sites); Figure\ S\@ref(fig:altitude-corg)), soil texture (Table\ S\@ref(fig:site-soil); Figure\ S\@ref(fig:clay-corg)) and history. First, we found that SOC stocks were positively correlated with elevation, both between and within sites (Figure\ S\@ref(fig:altitude-corg)). This result is consistent with those of @Raich2006, who found higher SOC at higher elevations in tropical evergreen forests in different countries (e.g. Congo, Thailand). As changes in elevation are associated with variations in climate (precipitation, temperature), our result is also consistent with previous studies that found higher SOC in areas with higher precipitation [@Schuur2001] and lower temperature [@Raich2006;@MarinSpiotta2013]. Second, soil texture is a key driving process for SOC storage in the tropics [@Don2010;@Wiesmeier2019;@Matus2021]. In particular, many studies have found a strong positive correlation between SOC stocks and the fine mineral fractions of soils [i.e. clay or clay + fine silt content; @Zinn2005]. In our study, we found no correlation between SOC stocks and clay + fine silt content when site and depth were included in the model (Figure\ S\@ref(fig:clay-corg)). Furthermore, the Gariuai soils, which had the highest stocks of soil organic carbon (SOC), were also the least clayey. This contradicts the typical (positive) relationship between the fine mineral fraction and SOC stocks. It is therefore unlikely that the difference in soil organic carbon (SOC) between the two sites can be attributed to soil texture. Thirdly, history could be a significant driving factor. Gariuai was continuously inhabited for over 50 years, whereas Osso-Luga was abandoned for almost three decades. The management and care of the AFSs in Gariuai, such as pruning and livestock visits, as well as optimizing the different strata in terms of C-input, could have led to an increase in soil organic carbon compared to abandoned systems [@Lorenz2014].

## SOC stocks are not explained by variation in AGC stocks

We found no significant relationship between AGC and SOC stocks both in the topsoil layer 0-10 cm depth (H2a refuted) and in 0-30 cm depth (H2b confirmed). These results may relate to previous findings that SOC stocks saturate much faster than AGC stocks in tropical AFS. For example, in a global meta-analysis,  @Ma2020 estimated that it took only 5 years for SOC stocks in tropical AFS to reach an equilibrium value, but more than three decades for AGC stocks. This desynchronisation of SOC and AGC stocks recovery has also been observed in naturally regenerating tropical forests. In a systematic review, @Martin2013 found that SOC in tropical secondary forests changed little with time after disturbance, while AGB increased. @Ojoatre2024 found similar results in a tropical montane secondary forest. These results imply that there is little relationship between AGC and SOC once agroforestry systems are more than 5 years old, which is the case in our study for FGs, HGs (> 5 years old) and partially for YAs (2 to 10 years; Table\ S\@ref(tab:afs-typology)). This saturation of SOC even with increasing AGC is counterintuitive, as it might be expected that greater AGC would lead to greater litter production and decomposition, some of which would increase SOC. However, @Sayer2019, found that SOC did not increase in a lowland tropical forest in Panama after 15 years of an experimentally doubled aboveground litter input. Two processes could explain this. On the one hand, the addition of easily decomposable litter could stimulate microbial decomposition and turnover of old stored SOC [i.e. priming effect; @Sayer2019]. On the other hand, the capacity of the soil to store SOC through organo-mineral associations would be limited by its fine element content (clay + fine silt) (i.e. saturation concept).

In addition to biomass stocks, other vegetation characteristics may account for variations in soil carbon stocks between AFSs. For instance, previous studies have shown that tree diversity can enhance SOC sequestration in natural ecosystems [@Spohn2023;@Chen2018] and in AFSs [@Manaye2021;@Islam2015]. Moreover, total soil organic carbon may be less sensitive to management practices such as biomass inputs than specific fractions of soil organic carbon [@Haynes2005]. Therefore, it would be interesting to continue with further studies that focus not only on the quantity but also the quality of soil organic matter and its relationship with biodiversity (flora and fauna) in traditional Timorese AFSs. 

## How to conserve and improve carbon stocks in traditional AFS in Timor Leste?  

Although agroforestry can make agriculture more resilient to climate change while improving its carbon footprint and biodiversity conservation value [@Cardinael2021], poor adaptation to the local context can explain many failures to adopt new agroforestry techniques [@Coe2014]. Conversely, some traditional agroforestry techniques have evolved in a particular context (biophysical, social and economic) and are therefore adapted to it [@Gouyon1993;@Aumeeruddy1994;@Peltier1996]. This is the case of the traditional AFS studied here: their heritage and socio-economic values [as shown by @Cogne2024], in addition to their high carbon stock value, confirm the importance of their conservation, both nationally and internationally.

Conservation of these traditional AFSs should avoid their abandonment as the trees wither or as their production value declines. Clear-cutting old agroforests for full-sun crops may then become an attractive short-term strategy, providing a large cash inflow from the sale of wood products, followed by 4 or 5 years of agricultural production on fertile land. Unfortunately, this results in a significant loss of biodiversity and carbon stored in the biomass, and can lead to an irreversible loss of fertility on the plot through soil degradation and erosion. This phenomenon has been documented in a similar context in Sumatra (Indonesia), where @Gouyon1993 highlighted the fact that old agroforests, which they called "jungle-rubber" and which are quite similar in structure to "forest gardens" (except for the rubber tree component, which has been replaced by other species in Timor), were being cleared in favor of more intensive production systems. This trend towards clearing jungle rubber was subsequently confirmed by @Ekadinata2011, and its impact on carbon stock decline was quantified by @Villamor2014.

Strategies to avoid clearing of these traditional AFS could be to develop new ways of intensifying production while maintaining high carbon and biodiversity values, and gradually renewing the tree population in the plots. For example, it would be interesting to develop renewal through clearings of about one hundred square meters (corresponding to the felling of 1 to 10 trees, depending on their crown area). In these clearings, young trees of species considered interesting for their future production would be associated, first with light crops (cereals, etc.) and then with crops of shady species (taro, etc.). Additional mechanisms to encourage the renewal of traditional AFS could be the provision of subsidies (e.g. payment of carbon credits) or low-interest loans to farmers wishing to renew their agroforestry plots. Another strategy, which has been successfully implemented in other tropical countries (e.g. for agroforestry coffee), is to improve the incomes of AFS farmers by creating 'sustainable agriculture'-type labels [@Bertrand2019], although this is primarily applicable to the production of export goods, whereas the AFSs we have studied are currently oriented more towards local consumption and domestic markets.

# Conclusion

Agroforestry systems in the Baucau district of Timor had variable but overall high carbon stocks. These systems have only recently been described in the scientific literature, and much remains to be done to improve our knowledge of their diversity and functioning. It is therefore important that all stakeholders including local authorities, universities and NGOs, take into account their existence, diversity and environmental and social value, in order to propose possible innovations to maintain or improve their functioning.
...
We are grateful to Marcal Gusmao (National University of Timor-Leste) and Alain Rival (Cirad) for their facilitation of the field study, and to the trainees who helped with field data collection: Juvencio Dos Santos (vegetation) and Teofilio Gomes (soil). We would also like to thank the 31 farmers who gave up their time to take part in the interviews and field measurements. 

This project was co-funded by the European Union and the German Federal Ministry for Economic Cooperation and Development (BMZ) as part of the Partnership for Sustainable Agroforestry (PSAF) - Ai ba Futuru project; the Deutsche Gesellschaft für Internationale Zusammenarbeit (GIZ); the French Embassy in Jakarta; and the French Institute of Timor-Leste.

# Data availability 

All data collected for this study is available in the following Dataverse repository ______.

The R codes used to analyse the data and produce the manuscript (including figures and tables) are available on ______.  

\newpage

# Tables

```{r tab.id = "summary", results = "show", tab.cap = "Mean and associated 95% confidence intervals (between parentheses) of total, aboveground carbon (AGC), belowground carbon (BGC), and soil organic carbon (SOC) stocks in each agroforestry system."}
...
```{r fig.id = "map-sites", fig.height = 5, fig.width = 7, fig.cap="Plot location. (a) Map of Timor-Leste with the location of the two study sites (Gariuai and Osso-Luga) in the Bacau district. (b) Location of the 15 plots in Gariuai; (c) Location of the 15 plots in Osso-Luga. Panels (b) and (c) share the same spatial scale. In (b) and (c), the plots are represented by dots whose color represents the agroforestry system it belongs to. The background color represents the elevation (m), from the Shuttle Radar Topography Mission (SRTM), specifically the hole-filled CGIAR-SRTM (90 m resolution)."}
...
```{r fig.id = "barplot", fig.height = 5, fig.width = 6, fig.cap = "Barplot of the amount of carbon in each AFS (CF: crop and fallow, SP: silvopasture, YA: young agroforest, HG: home garden, FG: forest garden), per site. AGC: Aboveground (biomass) carbon; BGC: belowground (biomass) carbon; SOC: soil organic carbon."}
## reorder factors 
...
flextable(
  ...
  values = c("Site", "Clay (%)", "Silt (%)", "Sand (%)", "SOC content (%)", 
             "N", "C/N", "pH", "CEC", "Ca", "K", "Mg", "Na"))|> 
  compose(i = 1, j = "N", part = "header", value = as_paragraph("N (%", as_sub("o"), ")")) |>
...
vern[, gsub("kamii", "kami", TETUM_NAME)]
vern[, gsub("ai-haek", "ai-hanek", TETUM_NAME)]
vern[, gsub("ai-bobur", "ai-bubur", TETUM_NAME)]
