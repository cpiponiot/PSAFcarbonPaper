...
packages_needed <- c(
  "ggplot2", "ggrepel",  "ggspatial",  "ggpubr",  "ggfortify",  "maps", 
  "rdryad", "readxl", "data.table", "knitr", "BIOMASS", "terra", "car", 
  "flextable"
)
...
vern1 <- lapply(sheets1, \(x){
  ...
  df <- df[, c("TETUM_NAME", "SCIENTIFIC_NAME")] |>
    unique() |>
    subset(!SCIENTIFIC_NAME %in% c("Unknown", "NR", "Unkown"))
})
...
g2 <- data_plot[, c("temp", "prec", "varp")] |>
  ...
  princomp() |>
  autoplot(
    data = data_plot, colour = "site", loadings.colour = "black",
