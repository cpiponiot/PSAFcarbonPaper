...
In addition, at one location at the center of the plot, soils were sampled with a cylinder of 250 cm^3^ at depths of 2-7 cm, 12-17 cm and 22-27 cm. Each sample was dried at 105°C in a classic oven until a stable dry mass was reached and then sieved to 2 mm. The fine fraction (\< 2 mm) was weighed to be used in the SOC stocks calculation (equation \@ref(eq:soc)).
...
$$SOC_p = \frac{\sum_i Corg_i \cdot (mfine_i \cdot 10^{-6}) \cdot (Splot \cdot \Delta d)}{Vcyl}$$

Where $Corg_i$ is the SOC content at depth $i$ (either 0-10, 10-20 or 20-30 cm); $mfine_i$ is the mass of the fine fraction of the soil in the sampling cylinder at depth $i$ (in g), converted into Mg with a factor of 10$^{-6}$; $Splot$ is the surface of a 1-ha plot (10$^8$ cm$^2$); $\Delta d$ is the difference in sample depths (= 10 cm) and $Vcyl$ is the volume of the sampling cylinder (250 cm$^3$). This calculation corresponds to the method recommended by @Poeplau2017 (M4), which correctly takes into account the coarse fraction of the soil. At depth $i$ = 10-20 cm, where SOC content was not measured (see "Soil measurements" section), we estimated SOC content as a function of SOC content at 0-10 cm and 20-30 cm, assuming a negative exponential model of SOC content decrease with soil depth [@Mishra2009; @Murphy2019]:

\begin{equation}
Corg_{10,20} = Corg_{0,10} \cdot \sqrt{\frac{Corg_{20,30}}{Corg_{0,10}}} (\#eq:intersoc)
\end{equation}
...
We calibrated the following models for AGC stocks of plot $i$ in site $k$ and AFS type $j$: 

$$AGC_{i,j,k}  = \mu_A + \alpha_{A,j} + \beta_{A,k} + \epsilon_{A,i}$$
with $\mu_A$ the average AGC stocks in our study area, $\alpha_{A,j}$ and  $\beta_{A,k}$ the random AFS and site effects, respectively distributed as: 

$$\alpha_{A,j} \sim \mathcal{N}(0,\sigma_{\alpha A}^2)$$

$$\beta_{A,k} \sim \mathcal{N}(0,\sigma_{\beta A}^2)$$
And $\epsilon_{A,i}$ the random error, distributed as: 

$$\epsilon_{A,i} \sim \mathcal{N}(0,\sigma_A^2)$$
$\sigma_A$, $\sigma_{\alpha A}$, and $\sigma_{\beta A}$ are the standard deviation of the random error, the AFS effect and the site effect respectively. 

Similarly, we modelled SOC stocks of plot $i$ in site $k$ and AFS type $j$ as: 

$$SOC_{i,j,k} = \mu_S +  \alpha_{Sj} + \beta_{Sk} + \theta_1 agc_i + \theta_2 fine_i+ \epsilon_{S,i}$$
with $\mu_S$ the average SOC stocks in our study area; $\theta_1$ the effect of AGC stocks on SOC stocks and $agc_i$ the standardised AGC stocks of plot $i$; $\theta_2$ the effect of fine soil fraction (clay + fine silt) on SOC stocks and $fine_i$ the standardised fine soil fraction of plot $i$. We standardised AGC stocks and fine soil fraction by scaling and centring values.  

$\alpha_{S,j}$ and $\beta_{S,k}$ are the random AFS and site effects, respectively distributed as: 

$$\alpha_{S,j} \sim \mathcal{N}(0,\sigma_{\alpha S}^2)$$

$$\beta_{S,k} \sim \mathcal{N}(0,\sigma_{\beta S}^2)$$
And $\epsilon_{S,i}$ the random error, distributed as: 

$$\epsilon_{S,i} \sim \mathcal{N}(0,\sigma_S^2)$$
$\sigma_S$, $\sigma_{\alpha S}$, and $\sigma_{\beta S}$ are the standard deviation of the random error, the AFS effect and the site effect respectively. 

Calibration was carried out using an adapted form of the Hamiltonian Monte Carlo using Stan’s programming language [@Carpenter2017]. We used the standard half-normal distribution as the prior for all standard deviation parameters, and the standard normal distribution for both $\theta_1$ and $\theta_2$ parameters. To propagate uncertainties on the model outputs, we calibrated the model for each of the 1000 iterations and sampled one set of parameter values in the posterior distributions for each iteration.   
...
model {
  ...
  for (i in 1:N) {
    pred_a[i] = mu_a + delta_afs_a[afs[i]] + delta_site_a[site[i]];
    pred_s[i] = mu_s + theta_agc_s*agc_sc[i] + theta_clay_s*clay_sc[i] + delta_afs_s[afs[i]] + delta_site_s[site[i]];
    pred_s10[i] = mu_s10 + theta_agc_s10*agc_sc[i] + theta_clay_s10*clay_sc[i] + delta_afs_s10[afs[i]] + delta_site_s10[site[i]];
  }
  ...
  //  priors
  theta_agc_s ~ normal(0,1);
  theta_agc_s10 ~ normal(0,1);
  theta_clay_s ~ normal(0,1);
  theta_clay_s10 ~ normal(0,1);
  sigma_a ~ normal(0,1);
  sigma_s ~ normal(0,1);
...
if (!file.exists("outputs/data_par.csv")) {
  ...
  clusterEvalQ(cl, {library(rstan); library(data.table)})
  clusterExport(cl, "data_sim"); clusterEvalQ(cl, data_sim)
  clusterExport(cl, "stan_model"); clusterEvalQ(cl, stan_model)
  
  data_par <- parLapply(cl, unique(data_sim$iter), function(i) {
    rstan_options(auto_write = TRUE)
    
    data_stan <- data_sim |> 
      subset(iter==i) |> 
      dcast(N_PLOT + site + ID_AF + fine_soil ~ variable) |> 
      with(
        list(
          N = length(N_PLOT),
          ...
          clay_sc = (fine_soil-mean(fine_soil))/sd(fine_soil),
          agc_sc = (AGC-mean(AGC))/sd(AGC),
          agc = AGC,
          soc = SOC,
          soc10 = SOC10
        )) 
  ...
  write.csv(data_par, file = "outputs/data_par.csv", row.names = FALSE)
  
  stopCluster(cl)
  
} else data_par <- read.csv("outputs/data_par.csv") |> setDT()

data_par[, effect := tstrsplit(param, "_")[[2]]]
data_par[! effect %in% c("afs", "site", "agc", "clay"), effect := NA]
...
df_mu_delta <- 
  ...
  dcast(iter + iter_stan + stock + effect + level ~ param) |> 
  merge(dcast(data_par[param == "mu"], iter + iter_stan + stock ~ param))
df_mu_delta[, `:=`(value = mu + delta, param = "pred_med")]
data_par <- rbind(
  data_par[, c("param", "iter", "iter_stan", "stock", "effect", "level", "value")], 
  df_mu_delta[, c("param", "iter", "iter_stan",  "stock", "effect", "level", "value")]
)
...
labels_effect = function(x) ifelse(x=="agc", "AGC", "clay + fine silt")
pars_summary |> 
  subset(param == "theta")  |> 
  ggplot(aes(y = ifelse(stock=="soc10", "0-10 cm", "0-30 cm"))) + 
  geom_vline(xintercept = 0, lty = 2) +
  geom_segment(aes(x = low95, xend = upp95)) + 
  geom_segment(aes(x = low80, xend = upp80), col = 2, lwd = 3) + 
  geom_point(aes(x = mean), size = 2) +
  facet_wrap(~effect, labeller = labeller(effect = labels_effect)) + 
...
dfpred <- expand.grid(agc = seq(0, 170, length.out = 20), 
  ...
  iter_stan = unique(data_par$iter_stan)
) |> 
  merge(
    data_par[grepl("soc", stock) & (param =="mu" | effect == "agc")] |> 
      dcast(iter + iter_stan + stock ~ param)
  ) |> 
  merge(
...
dfpred[, depth := ifelse(grepl(10, stock), "Soil depth: 0-10 cm", "Soil depth: 0-30 cm")]
