...
[paragraph on possible drivers]
C stocks (in biomass and soil) of AFS could be driven by a number of factors: 
- agricultural practices (ref)
- climate (ref)
- soil properties (especially for SOC) > in particular, fine content
...
```{r get-env-data}
# temperature and precipitation from CHELSA
...
cor(data_plot[, c("elev", "prec", "temp")])
...
where $Corg_{a,b}$ is the average SOC content between depths a and b (see rationale for equation \@ref(eq:intersoc) in Supplementary Information xx).
...
data_soil_summ[, fine_soil := CLAY + `FINE SILT`] 
data_sim <- merge(data_sim, data_soil_summ)  
# add elevation
data_sim <- merge(data_sim, data_plot[, c("N_PLOT", "elev")])
```
...
We modelled both AGC and SOC stocks as: 

$$Y_{i,j,k} = \mu +  \theta \cdot X_i + \alpha_{j} + \beta_{k} + \epsilon_{i}$$
with $Y_{i,j,k}$ the stocks (either SOC or AGC) of plot $i$ in site $k$ and AFS type $j$

$\mu$ is the average stocks in our study area.

$\theta$ is the effect of the covariates on $Y$, and $X_i$ are the values of the covariates in plot $i$; they include elevation for AGC stocks [from NASA SRTM; @SRTM2024], or elevation [@SRTM2024], clay + fine soil content (from soil analysis; xxx methods) and AGC stocks for SOC stocks. All covariates were centred and scaled. Elevation was chosen as it was highly correlated (>95%) with both mean annual temperature and precipitation [@Karger2017]: it was thus a good indicator of a climate gradient in our dataset (see Supplementary information section xx). xxx justify these variables in the introduction
...
model {
  ...
  // predictions
  for (i in 1:N) {
    pred_a[i] = mu_a + elev[i] * theta_a + delta_afs_a[afs[i]] + delta_site_a[site[i]];
    pred_s[i] = mu_s + dot_product(row(covar, i), theta_s) + delta_afs_s[afs[i]] + delta_site_s[site[i]];
    pred_s10[i] = mu_s10 + dot_product(row(covar, i), theta_s10) + delta_afs_s10[afs[i]] + delta_site_s10[site[i]];
  }
  
  // likelihoods
  agc ~ normal(pred_a, sigma_a);
  soc ~ normal(pred_s, sigma_s);
  soc10 ~ normal(pred_s10, sigma_s);
...
if (!file.exists("outputs/data_par.csv")) {
  ...
  for (n in setdiff(seq_len(100), pars_prev)) {
    data_par <- lapply(paste0("V", seq((n-1)*10+1, n*10)), function(i) {
      ...
      data_stan <- data_sim |> 
        subset(iter==i) |> 
        dcast(N_PLOT + site + ID_AF + fine_soil + elev ~ variable) |> 
        with(
          list(
            N = length(N_PLOT),
            ...
            covar = scale(cbind(elev, fine_soil, AGC)),
            agc = AGC,
            soc = SOC,
            soc10 = SOC10
          )) 
...
data_par[param == "theta",
         level := c("elev", "fine", "agc")[level_num]]
data_par[param == "theta" & stock == "agc", level := "elev"]
...
## Effect of elevation and fine soil on AGC and SOC stocks

## Relationship between AGC and SOC stocks

The effect of AGC on SOC was close to zero (Figure \@ref(fig:plot-reg)). 
...
```{r fig.id = "effect-afs-site", fig.width = 7, fig.height = 3}
...
```{r dag, fig.cap="Direct acyclic plot of the hypothesised effect of fine soil (clay + fine silt content), climate (through elevation) and land use (here AFS) on AGC and SOC stocks."}
